...
class TestGetMarketDataCache:
    ...
    def test_miss_calls_underlying_function(self):
        mock_fn = MagicMock(return_value="price data")
        with patch.object(mcp_cache, "_orig_get_market_data", mock_fn):
    ...
    def test_hit_skips_underlying_function(self):
        ...
        with patch.object(mcp_cache, "_orig_get_market_data", mock_fn):
            ...
            cached_get_market_data("AAPL")
        mock_fn.assert_called_once()  # only the first call
...
class TestGetMarketOverviewCache:
    ...
    def test_miss_calls_underlying_function(self):
        mock_fn = MagicMock(return_value="overview data")
        with patch.object(mcp_cache, "_orig_get_market_overview", mock_fn):
...
class TestAnalyzePortfolioCache:
    ...
    def test_miss_calls_underlying_function(self):
        mock_fn = MagicMock(return_value="portfolio report")
        with patch.object(mcp_cache, "_orig_analyze_portfolio", mock_fn):
            result = cached_analyze_portfolio("50% VOO, 50% BND")
        assert result == "portfolio report"
        mock_fn.assert_called_once_with("50% VOO, 50% BND")
...
class TestLookupExpenseRatioCache:
    ...
    def test_miss_calls_underlying_function(self):
        mock_fn = MagicMock(return_value="0.03% expense ratio")
        with patch.object(mcp_cache, "_orig_lookup_expense_ratio", mock_fn):
            result = cached_lookup_expense_ratio("VOO")
        assert result == "0.03% expense ratio"
        mock_fn.assert_called_once_with("VOO")
...
class TestExtractTickerCache:
    ...
    def test_miss_calls_underlying_function(self):
        with patch.object(mcp_cache, "_orig_extract_ticker", return_value="AAPL") as mock_fn:
    ...
    def test_returns_none_when_no_ticker(self):
        with patch.object(mcp_cache, "_orig_extract_ticker", return_value=None):
...
class TestKeyNormalisation:
    ...
    def test_expense_ratio_fund_is_normalised(self):
        ...
        mock_fn = MagicMock(return_value="0.03%")
        with patch.object(mcp_cache, "_orig_lookup_expense_ratio", mock_fn):
            cached_lookup_expense_ratio("voo")
            cached_lookup_expense_ratio("VOO")
            cached_lookup_expense_ratio(" VOO ")
...
class TestCacheIsolation:
    ...
    def test_caches_are_independent(self):
        ...
        mock_market = MagicMock(return_value="market data")
        mock_overview = MagicMock(return_value="overview")
        with patch.object(mcp_cache, "_orig_get_market_data", mock_market), \
             patch.object(mcp_cache, "_orig_get_market_overview", mock_overview):
            cached_get_market_data("AAPL")
            cached_get_market_overview()
